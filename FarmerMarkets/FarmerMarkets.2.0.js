...
globalConfig.legend = {
	available: true,	
	url: "legend_en.png", 
	size: {width: 238, height: 108},   //Width and Height
	location: {ratioX: 0.01, ratioY: 0.25}  //
};
/*	
globalConfig.createTable = function (features) {
	var str = '<h2>' + features.length + globalConfig.chooseLang(' large FarmerMarketss found in Ontario', ' sites trouvés Ontario')  + '</h2><table id="large-FarmerMarketss">\
		<thead>\
	...
	for (var i=0; i<features.length; i++) {
		str = str + '<tr><td>' + features[i].attributes.COFA_NUM  + '</td><td><a href="http://www.ontario.ca/environment-and-energy/large-FarmerMarkets-site-details?site=' + features[i].attributes.COFA_NUM + '">' + features[i].attributes.SITE_NAME + '</a></td></tr>';
	}
	str = str + '</tbody></table>';
